...
```rust
// Core types
pub struct State<T>(Arc<RwLock<T>>);  // Async-safe with interior mutability
...
impl Commands {
    pub async fn execute(self, state: sen::State<AppState>) -> sen::Response {
        ...
        match self {
            Commands::Preflight => {
                handlers::preflight(state).await.into_response()
            }
            Commands::Serve(args) => {
                handlers::serve(state, args).await.into_response()
            }
            Commands::Build(args) => {
                handlers::build(state, args).await.into_response()
            }
        }
    }
}
...
pub async fn preflight(state: State<AppState>) -> CliResult<String> {
    // Access state asynchronously
    let config = state.read().await;
    Ok("Preflight checks passed".to_string())
}

// Pattern 2: State + Args (Axum-style, order doesn't matter)
pub async fn serve(state: State<AppState>, args: ServeArgs) -> CliResult<String> {
...
pub async fn build(args: BuildArgs, state: State<AppState>) -> CliResult<()> {
    // Order is flexible
    Ok(())
}
```
...
async fn cleanup(state: State<AppState>) -> CliResult<()>

// Return custom type implementing IntoResponse
async fn report(state: State<AppState>) -> CliResult<Report>
```
...
```rust
// ✅ Valid signatures
async fn handler1(state: State<App>, args: Args) -> CliResult<String>
async fn handler2(args: Args, state: State<App>) -> CliResult<String>
async fn handler3(state: State<App>) -> CliResult<String>
async fn handler4(args: Args) -> CliResult<String>  // No state needed

// ❌ Invalid signatures (compile error)
async fn handler5(state1: State<App>, state2: State<App>) -> CliResult<String>
async fn handler6(args1: Args, args2: Args) -> CliResult<String>
```
...
pub fn derive_sen_router(input: TokenStream) -> TokenStream {
    ...
    quote! {
        impl #enum_name {
            pub async fn execute(self, state: State<#state_type>) -> Response {
                match self {
                    #(#match_arms)*
                }
            }
...
async fn main() {
    ...
    // 4. Execute command (macro-generated async)
    let response = cli.execute(State::new(app_state)).await;
...
pub async fn deploy(state: State<AppState>, args: DeployArgs) -> CliResult<String> {
    ...
    // 2. Delegate to workflow (async)
    let report = workflows::deploy::execute(&state, &args.env).await?;
...
```rust
// ❌ Bad: Direct output
pub async fn status(state: State<AppState>) -> CliResult<()> {
...
pub async fn status(state: State<AppState>) -> CliResult<StatusReport> {
